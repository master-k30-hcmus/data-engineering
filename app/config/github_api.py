...
class GithubAPI(object):
  ...
  def get_info(self, repo_name):
    ...
    status, headers, data = self._call_api(method='GET', url=f'https://api.github.com/repos/{repo_name}', headers=headers)
    return(data)
  ...
  def get_repo_feature(self, repo_name, feature):
    ...
    """
    headers = {"Authorization": f"token {self.token}"}
    status, headers, data = self._call_api(method='GET', url=f'https://api.github.com/repos/{repo_name}/{feature}', headers=headers)
    return(data)
